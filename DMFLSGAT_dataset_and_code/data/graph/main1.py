...
if __name__ == '__main__':
    ...
    import torch
    from graph.utils1 import load_data_with_smiles, set_seed, load_fp
    from torch.utils.data import DataLoader
    from utils1 import SmilesUtils, MyDataset
    import pandas as pd
    ...
    plt.tight_layout()
    plt.savefig("confusion_matrix_gat.png", dpi=300)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("roc_curve_gatpng", dpi=300)
    plt.show()
    ...
    exp = explainer.explain_instance(
        to_explain_vector,
        lime_predict,
        num_features=20,   # nombre de features affichées
        top_labels=1
    )
    ...
    fig.tight_layout()
    plt.show()
    fig.savefig("lime_explanation_sample_gat.png")
